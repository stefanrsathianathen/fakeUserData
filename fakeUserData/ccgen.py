...
EDITED BY STEFAN SATHIANATHEN MAY 18 2017
"""
...
def credit_card_number(prefixList, length, howMany=1):
    result = []
...
if __name__ == "__main__":
    ...
    visa16 = credit_card_number(visaPrefixList, 16)
    print(output("VISA 16 digit", visa16))
    visa13 = credit_card_number(visaPrefixList, 13)
    print(output("VISA 13 digit", visa13))
    amex = credit_card_number(amexPrefixList, 15)
    print(output("American Express", amex))

    
    # Minor cards
    discover = credit_card_number(discoverPrefixList, 16)
    print(output("Discover", discover))
    diners = credit_card_number(dinersPrefixList, 14)
    print(output("Diners Club / Carte Blanche", diners))
    enRoute = credit_card_number(enRoutePrefixList, 15)
    print(output("enRoute", enRoute))
    jcb15 = credit_card_number(jcbPrefixList15, 15)
    print(output("JCB 15 digit", jcb15))
    jcb16 = credit_card_number(jcbPrefixList16, 16)
    print(output("JCB 16 digit", jcb16))
    voyager = credit_card_number(voyagerPrefixList, 15)
